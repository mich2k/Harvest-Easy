...
def report(id_bin, db, filling=None, coord=None, co2=None):
    ...
    get_user_from_apartment = db.session.query(
        User.username).where(User.apartment_ID == apartment)
    get_admin_from_apartment = db.session.query(
        Apartment.associated_admin).where(Apartment.apartment_name == apartment)

    chat_ids = db.session.query(UserTG.id_chat).filter(
        UserTG.associated_user.in_(get_user_from_apartment)).all()
    chat_ids.append(db.session.query(UserTG.id_chat).filter(
        UserTG.associated_user.in_(get_admin_from_apartment)).all())
