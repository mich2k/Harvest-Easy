...
def getmap():
    ...
    for apartment in apartments:
        ...
        for bin in bins:
            ...
            ultimo_bin_record = BinRecord.query.filter(
                BinRecord.associated_bin == bin.id_bin).order_by(BinRecord.timestamp.desc()).first()
            ...
            point['status'] = status
            point['id'] = bin.id_bin
            point['address'] = apartment.street + " " + \
                str(apartment.apartment_street_number) + ", " + apartment.city
    ...
    with open('./out/points.json', 'w') as outfile:
