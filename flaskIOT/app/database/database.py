...
class Bin(db.Model):
    ...
    id_bin = db.Column('bin_id', db.Integer, default=1) 
    idcondominio = db.Column('condominio_id', db.Integer, default=10)
    ...
    def __init__(self, status, temperature, umidità, co2, riempimento):
        self.status = status
...
class Utente(db.Model):
    ...
    idcondominio = db.Column('condominio_id', db.Integer, primary_key=True, default=10)
...
def additem():
    ...
    umidità=request.args.get('umidità')
    co2=request.args.get('co2')
    riempimento=request.args.get('riempimento')
    ...
    sf = Bin(status_attuale, temperature, umidità, co2, str(riempimento))
