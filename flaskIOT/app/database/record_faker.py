...
def faker_instances():
    ...
    next_timestamp = utils.Utils.randomTime(rdm=False) if act_record is None else utils.Utils.randomTime()
    next_filling = act_filling.riempimento + utils.Utils.get_limited_random(0.1, 0.2) if act_filling is not None else utils.Utils.get_limited_random(0, 0.1)
    next_status = 1
    
    if act_record is not None:
        diff = datetime.strptime(act_record.timestamp, "%Y-%m-%d %H:%M:%S") - datetime.strptime(next_timestamp, "%Y-%m-%d %H:%M:%S")
